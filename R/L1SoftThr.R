#' @title L1SoftThr - Iterative Soft Thresholding Algorithm based on \eqn{l_{1,1}} norm
#' @description The function aims to solve \eqn{l_{1,1}} regularized least squares.
...
#' @return The solution of proximal gradient method with \eqn{l_{1,1}} regularizer.
#' @author Xinlin Hu <thompson-xinlin.hu@connect.polyu.hk>
#'
#' Yaohua Hu <mayhhu@szu.edu.cn>
#' @details The L1SoftThr function aims to solve the problem:
#' \deqn{\min \|AX-B\|_F^2 + \lambda \|X\|_{1,1}}
...
#'
L1SoftThr <- function(A, B, X, s, maxIter = 200){
  # Initialization
  v <- 0.5 # stepsize
  Bu1 <- 2 * v * t(A) %*% B
  Bu2 <- 2 * v * t(A) %*% A
  ...
  for(k in 1:maxIter){
    ...
    for(i in ind){
      ...
      # Update matrix
      Xnew[i, ] <- (1 - v * lambda / normRow) * rowDa
    }
