#' @title L1HalfThr - Iterative Half Thresholding Algorithm based on \eqn{l_{1,1/2}} norm
#' @description The function aims to solve \eqn{l_{1,1/2}} regularized least squares.
...
#' @return The solution of proximal gradient method with \eqn{l_{1,1/2}} regularizer.
#' @author Xinlin Hu <thompson-xinlin.hu@connect.polyu.hk>
#'
#' Yaohua Hu <mayhhu@szu.edu.cn>
#' @details The L1HalfThr function aims to solve the problem:
...
#'
L1HalfThr <- function(A, B, X, s, maxIter = 200){
  # Initialization
  v <- 0.5 # stepsize
  Bu1 <- 2 * v * t(A) %*% B
  Bu2 <- 2 * v * t(A) %*% A
  ...
  for(k in 1:maxIter){
    ...
    for(i in ind){
      rowDa <- Bu[i, ]
      L1Row <- L1normFun(rowDa)
      eta <- q * (3/L1Row)^(0.5)
      phi <- acos(q * n * (3/L1Row)^(1.5))

      # Update matrix
      Xnew[i, ] <- rowDa - eta / cos((pi - phi) / 3) * sign(rowDa)
    }
