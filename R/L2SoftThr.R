#' @title L2SoftThr - Iterative Soft Thresholding Algorithm based on \eqn{l_{2,1}} norm
...
#' @return The solution of proximal gradient method with \eqn{l_{2,1}} regularizer.
#' @author Xinlin Hu <thompson-xinlin.hu@connect.polyu.hk>
#'
#' Yaohua Hu <mayhhu@szu.edu.cn>
#' @details The L2SoftThr function aims to solve the problem:
...
#'
L2SoftThr <- function(A, B, X, s, maxIter = 200){
  # Initialization
  v <- 0.5 # stepsize
  Bu1 <- 2 * v * t(A) %*% B
  Bu2 <- 2 * v * t(A) %*% A
