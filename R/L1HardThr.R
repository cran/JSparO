#' @title L1HardThr - Iterative Hard Thresholding Algorithm based on \eqn{l_{1,0}} norm
#' @description The function aims to solve \eqn{l_{1,0}} regularized least squares.
...
#' @return The solution of proximal gradient method with \eqn{l_{1,0}} regularizer.
#' @author Xinlin Hu <thompson-xinlin.hu@connect.polyu.hk>
#'
#' Yaohua Hu <mayhhu@szu.edu.cn>
#' @details The L1HardThr function aims to solve the problem:
#' \deqn{\min \|AX-B\|_F^2 + \lambda \|X\|_{1,0}}
...
#'
L1HardThr <- function(A, B, X, s, maxIter = 200){
  # Initialization
  v <- 0.5 # stepsize
  Bu1 <- 2 * v * t(A) %*% B
  Bu2 <- 2 * v * t(A) %*% A
